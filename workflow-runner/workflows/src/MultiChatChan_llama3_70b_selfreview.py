...
class MultiChatChain:
    ...
    def stream(self, input) -> Generator[str, None, None]:
        ...
        with ThreadPoolExecutor() as executor:
            intro_future = executor.submit(self._get_intro_text, input)
            response_future = executor.submit(self._get_response_text, input)

            intro = intro_future.result()
            ...
            for i in range(self.number_of_reviews):
                if i == self.number_of_reviews - 1:
                    ...
                    yield "\n\n"
                    response_future = executor.submit(self._get_review_text, response)
    ...
    def _get_default_intro(self):
        return {
            "intro": "Your question is now being answered by multiple AI models. Please wait for a while.",
